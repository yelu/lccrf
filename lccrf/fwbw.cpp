...
{
	// update nodes.
	for (auto xFeatureIte = x.Features.begin(); xFeatureIte != x.Features.end(); xFeatureIte++)
	{
		auto labelsAndFId = lccrfFeatures.UnigramFeatures.at(xFeatureIte->first);
		auto positions = xFeatureIte->second;

		for (auto labelIte = labelsAndFId->begin(); labelIte != labelsAndFId->end(); labelIte++)
		{
			double fWeight = weights[labelIte->second];
			int fId = labelIte->first;
			for (auto posIte = positions->begin(); posIte != positions->end(); posIte++)
			{
				nodes(*posIte, fId) += fWeight;
			}
		}	
	}
...
double FWBW::CalcNodesExpectation(const int& s2, const std::unordered_set<uint16_t>& positions)
{
    double res = 0.0;
